/**
 * Idempotent check-and-apply for the public.sources table (source configs → Supabase).
 *
 *  1. Checks whether public.sources already exists (it may — this app points at the same
 *     Supabase project as the old Next.js app, which ran this migration).
 *  2. If missing, applies scripts/sources-supabase-migration.sql over DIRECT_URL (session mode,
 *     the connection Prisma DDL must use — never the 6543 transaction pooler).
 *  3. Verifies RLS: 4 policies + rowsecurity enabled.
 *
 * Run:  npx tsx scripts/apply-sources-migration.mts     (from server/)
 */
...
try {
  ...
  const existsRows = await prisma.$queryRawUnsafe<{ exists: boolean }[]>(
    "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sources') AS exists",
  );
  const existedBefore = existsRows[0]?.exists === true;
  console.log(existedBefore ? "public.sources ALREADY EXISTS" : "public.sources is MISSING — applying migration");

  // 2. Apply (idempotent) if missing. Safe to re-run either way, but we only touch DDL when needed.
  if (!existedBefore) {
    const sql = readFileSync(path.join(here, "sources-supabase-migration.sql"), "utf8");
  ...
  const policyRows = await prisma.$queryRawUnsafe<{ count: bigint }[]>(
    "SELECT count(*)::bigint AS count FROM pg_policies WHERE tablename = 'sources'",
  );
  const rlsRows = await prisma.$queryRawUnsafe<{ on: boolean }[]>(
    "SELECT relrowsecurity AS on FROM pg_class WHERE relname = 'sources'",
  );
