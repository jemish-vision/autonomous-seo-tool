/**
 * Idempotent check-and-apply for the public.comparisons table (saved comparisons -> Supabase).
 *
 *  1. Checks whether public.comparisons already exists.
 *  2. If missing, applies scripts/comparisons-migration.sql over DIRECT_URL (session mode, the
 *     connection Prisma DDL must use — never the 6543 transaction pooler).
 *  3. Verifies RLS: 4 policies + rowsecurity enabled.
 *
 * Run:  npx tsx scripts/apply-comparisons-migration.mts     (from server/)
 */
...
try {
  ...
  const existsRows = await prisma.$queryRawUnsafe<{ exists: boolean }[]>(
    "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'comparisons') AS exists",
  );
  const existedBefore = existsRows[0]?.exists === true;
  console.log(existedBefore ? "public.comparisons ALREADY EXISTS" : "public.comparisons is MISSING — applying migration");

  // 2. Apply (idempotent) if missing. Safe to re-run either way, but we only touch DDL when needed.
  if (!existedBefore) {
    const sql = readFileSync(path.join(here, "comparisons-migration.sql"), "utf8");
  ...
  const policyRows = await prisma.$queryRawUnsafe<{ count: bigint }[]>(
    "SELECT count(*)::bigint AS count FROM pg_policies WHERE tablename = 'comparisons'",
  );
  const rlsRows = await prisma.$queryRawUnsafe<{ on: boolean }[]>(
    "SELECT relrowsecurity AS on FROM pg_class WHERE relname = 'comparisons'",
  );
  ...
  const cols = await prisma.$queryRawUnsafe<{ column_name: string }[]>(
    "SELECT column_name FROM information_schema.columns WHERE table_schema='public' AND table_name='comparisons' ORDER BY ordinal_position",
  );
