/**
 * Idempotent check-and-apply for the public.exports table (dataset export metadata → Supabase).
 *
 *  1. Checks whether public.exports already exists.
 *  2. If missing, applies scripts/exports-migration.sql over DIRECT_URL (session mode, the
 *     connection Prisma DDL must use — never the 6543 transaction pooler).
 *  3. Verifies RLS: 4 policies + rowsecurity enabled.
 *  4. Prints the resulting column list for a shape sanity check.
 *
 * Run:  npx tsx scripts/apply-exports-migration.mts     (from server/)
 *
 * NOTE: not run against the live DB by the porting work — apply it yourself when ready.
 */
...
try {
  ...
  const existsRows = await prisma.$queryRawUnsafe<{ exists: boolean }[]>(
    "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'exports') AS exists",
  );
  const existedBefore = existsRows[0]?.exists === true;
  console.log(existedBefore ? "public.exports ALREADY EXISTS" : "public.exports is MISSING — applying migration");

  // 2. Apply (idempotent) if missing. Safe to re-run either way, but we only touch DDL when needed.
  if (!existedBefore) {
    const sql = readFileSync(path.join(here, "exports-migration.sql"), "utf8");
  ...
  const policyRows = await prisma.$queryRawUnsafe<{ count: bigint }[]>(
    "SELECT count(*)::bigint AS count FROM pg_policies WHERE tablename = 'exports'",
  );
  const rlsRows = await prisma.$queryRawUnsafe<{ on: boolean }[]>(
    "SELECT relrowsecurity AS on FROM pg_class WHERE relname = 'exports'",
  );
  ...
  const cols = await prisma.$queryRawUnsafe<{ column_name: string }[]>(
    "SELECT column_name FROM information_schema.columns WHERE table_schema='public' AND table_name='exports' ORDER BY ordinal_position",
  );
