 ...
 *
 * Two hard facts from Google shape everything here: data is only final after about three days, and
 * history stops at 16 months. A picker that lets someone choose yesterday or two years ago produces
 * an empty chart that looks like a bug, so ranges are clamped to what can actually return data and
 * the clamp is reported rather than applied silently.
 *
 * Ported verbatim from poc/seo-dashboard/lib/gsc/date-range.ts (pure, no adaptation needed).
 */
...
/**
 * Validates and clamps a requested range, falling back to the default window rather than erroring
 * on malformed input.
 */
